import { Link, useLocation } from "react-router-dom";

export default function AccountNavigation() {
  const { pathname } = useLocation();
  let subpage = pathname.split("/")?.[2];
  function linkClasses(type = null) {
    if (type === "aboutroommate") {
    }
    const isActive =
      pathname === "/account/aboutroommate" && type === "aboutroommate";
    let classes =
      "w-[340px] h-[50px] flex items-center justify-center rounded-[5px] font-medium text-[20px]";
    if (type === subpage) {
      classes += " bg-[#33FF00] text-black";
    } else {
      classes += " bg-transparent border-[1px] border-[white] text-white";
    }
    return classes;
  }
  return (
    <nav className="max-w-[1200px] px-[20px] mx-[auto] items-center flex justify-between mt-[50px] gap-2 mb-[60px]">
      {/* <div className="flex items-center gap-[10px]">
        <Link
          className={linkClasses("aboutroommate")}
          to={"/account/aboutroommate"}
        >
          Иду на подселение
        </Link>
        <Link
          className={linkClasses("findroommate")}
          to={"/account/findroommate"}
        >
          Ищу сожителя
        </Link>
      </div> */}
      {subpage === "aboutroommate" ? (
        <Link
          to={"/account"}
          className="px-[5px] text-[#33FF00] font-semibold flex items-center gap-[5px]"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="currentColor"
            class="size-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z"
            />
          </svg>
          Заполнить анкету
        </Link>
      ) : (
        <Link
          to={"/account/findroommate/new"}
          className="px-[10px] py-[5px] rounded-[5px] text-[#161C24] bg-[#33FF00] font-semibold flex items-center gap-[5px]"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke-width="1.5"
            stroke="#161C24"
            class="size-6"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
            />
          </svg>
          Создать объявление
        </Link>
      )}
    </nav>
  );
}
